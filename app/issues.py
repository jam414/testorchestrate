...
def insert_new_issue(dbconn, project_id, testname, severity, type, framework, runner, tag, platform, precondition, steps):
...
def update_issue(dbconn, name, type, severity, project_id, req_id):
    cursor = dbconn.cursor()
    resp =''
    print(req_id)
    # sql = "INSERT INTO tx_requirements(?,?,?) VALUES(name, )"
...
def load_issues(dbconn, project_id, project_name):
    cursor = dbconn.cursor()
    resp =''
    # sql = "INSERT INTO tx_requirements(?,?,?) VALUES(name, )"
    # sql = "INSERT INTO tx_requirements(reqid, reqname, reqdesc, reqtype, reqpriority, reqseverity), values (123, name, 'SOme desc', type, 'Critical',severity)"
    ...
    #         '''
    sql1 = '''
                select defect_id, summary, description, assignee, status, priority 
                FROM tx_defects
                WHERE project_id = %s order by defect_id asc
            '''
