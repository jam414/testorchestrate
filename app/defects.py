...
def insert_new_defect(dbconn, project_id, testname, severity, type, framework, runner, tag, platform, precondition, steps):
...
def update_defect(dbconn, name, type, severity, project_id, req_id):
    cursor = dbconn.cursor()
    resp =''
    print(req_id)
    # sql = "INSERT INTO tx_requirements(?,?,?) VALUES(name, )"
...
def load_defects(dbconn, project_id, project_name):
    cursor = dbconn.cursor()
    resp =''
    # sql = "INSERT INTO tx_requirements(?,?,?) VALUES(name, )"
    # sql = "INSERT INTO tx_requirements(reqid, reqname, reqdesc, reqtype, reqpriority, reqseverity), values (123, name, 'SOme desc', type, 'Critical',severity)"
